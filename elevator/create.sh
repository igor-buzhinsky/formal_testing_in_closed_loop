...
for ((floors = from; floors <= to; floors++)); do
    ...
    echo "ltl pos$((floorstimes - posperfloor))_1 { X( []((elevator_pos == $((floorstimes - posperfloor))) && down && !up -> X(elevator_pos == $((floorstimes - posperfloor - 1)))) ) }" >> tmp
    echo "ltl pos$((floorstimes - posperfloor))_2 { X( []((elevator_pos == $((floorstimes - posperfloor))) && !(down && !up) -> X(elevator_pos == $((floorstimes - posperfloor)))) ) }" >> tmp

    echo >> tmp
    for ((i = 0; i < $floors; i++)); do
        echo "ltl floor$i { X( [] (on_floor[$i] <-> (elevator_pos == $((posperfloor * i)))) ) }" >> tmp
    done
